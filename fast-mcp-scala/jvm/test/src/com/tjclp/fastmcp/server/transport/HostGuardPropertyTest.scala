...
/** DRAFT (C3 step 10, TJC-2328) — property coverage of [[HostGuard]] and
  * [[HttpRequestGuards.postGate]] with ScalaCheck 1.18.1 (declared in `package.mill` but unused by
  * every other jvm test). Plain `org.scalacheck.Test.check` inside ScalaTest cases: the build
  * declares no `scalatestplus-scalacheck` bridge.
  *
  * Invariants pinned here (the coverage map S1.8 asked for them): fail-closed origin parsing
  * (userinfo, path, query, fragment, whitespace, bad ports 0 / 65536 / non-digits), case
  * insensitivity of both headers, the `[::1]` bracket rule (HostGuard.scala:167-174), the
  * `allowedOrigins` normalisation, `postGate` ordering (403 before everything), charset parameters
  * and Accept wildcards.
  *
  * Two cases are RED on main 8b2fede by construction and pin backlog items rather than today's
  * behaviour — each says so in its name:
  *   - a duplicated `Host` header (joined value) is admitted when its FIRST hostname is listed
  *     (D3.7, P3): `hostnameOf` truncates `"127.0.0.1:8000, evil.example.com"` at the first `:`;
  *   - a malformed `Host` port with no `Origin` degrades to "port-less" and is admitted although
  *     HostGuard.scala:20-22 / CHANGELOG.md:68 claim fail-closed on the Host side (D4.6, P3 — the
  *     alternative resolution is rewording the scaladoc, in which case that case is dropped).
  */
