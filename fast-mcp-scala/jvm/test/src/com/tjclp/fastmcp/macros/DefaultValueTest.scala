...
class DefaultValueTest extends AnyFunSuite with Matchers {
  ...
  case class UserConfig(
      ...
      preferences: Option[Map[String, String]] = Some(
        Map.empty
  ...
  test("fields with Option types (even with defaults) should not be marked as required") {
    ...
    assert(
      configProperties.hcursor.downField("username").focus.isDefined,
      "username field should be present"
    )
    assert(
      configProperties.hcursor.downField("email").focus.isDefined,
      "email field should be present"
    )
    assert(configProperties.hcursor.downField("age").focus.isDefined, "age field should be present")
    assert(
      configProperties.hcursor.downField("isActive").focus.isDefined,
      "isActive field should be present"
    )
    assert(
      configProperties.hcursor.downField("preferences").focus.isDefined,
      "preferences field should be present"
    )
  ...
  test("functions with mixed required/optional parameters should mark only required ones") {
    ...
    assert(
      properties.hcursor.downField("requiredParam").focus.isDefined,
      "requiredParam should be present in properties"
    )
    assert(
      properties.hcursor.downField("optionalParam").focus.isDefined,
      "optionalParam should be present in properties"
    )
    ...
    assert(
      !required.contains("optionalParam"),
      "optionalParam is an Option and should not be required"
    )
  }
}
