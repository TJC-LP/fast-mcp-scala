...
/** A request-pipeline stage. Middlewares compose around the terminal handler in registration order:
  * the first-listed middleware is outermost (runs first on the way in, last on the way out). The
  * canonical chain is validation → task augmentation → handler → error mapping.
  */
trait Middleware[R]:
