...
/** Wire shape of a prompt descriptor as returned in `prompts/list`.
  *
  * Distinct from [[com.tjclp.fastmcp.core.PromptDefinition]] (the user-facing registration
  * shape). Adds spec fields (`title`, `icons`, `_meta`) the registration form doesn't carry.
  */
case class Prompt(
    name: String,
...
/** A message returned as part of `prompts/get`. Distinct from
  * [[com.tjclp.fastmcp.core.Message]] (which is also user-facing) because `PromptMessage.content`
  * is a single `Content`, never a sequence.
  */
