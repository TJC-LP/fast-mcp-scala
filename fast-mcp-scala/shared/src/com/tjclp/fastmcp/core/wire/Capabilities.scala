...
/** Capabilities the server advertises on `initialize`.
  *
  * Per the design doc, this is **derived from the registered handler map** at runtime — the
  * router opts in `tools`/`resources`/`prompts`/etc. only when handlers are wired. Fixes issue
  * #56 by construction: `logging` is `None` unless a logging hook is registered.
  */
...
object ClientTasksElicitationRequest:
  given JsonCodec[ClientTasksElicitationRequest] = DeriveJsonCodec.gen[ClientTasksElicitationRequest]
