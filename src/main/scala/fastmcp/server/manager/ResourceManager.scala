 ...
 *
 * Since we assume we are always scheme-based, e.g. "users://{id}/profile",
 * the match patterns are anchored with ^ and $ so only exact matches pass.
 */
class ResourceManager extends Manager[ResourceDefinition]:
  ...
  private val resourceTemplates = new ConcurrentHashMap[String, (ResourceDefinition, ResourceTemplateHandler)]()
  ...
  def addResource(uri: String, handler: ResourceHandler, definition: ResourceDefinition): ZIO[Any, Throwable, Unit] =
    ZIO.attempt {
      ...
      if resources.containsKey(uri) then
         ...
         java.lang.System.err.println(s"[ResourceManager] Warning: Static resource with URI '$uri' already exists. Overwriting.")
   ...
   *
   * @param uriPattern Resource URI pattern with placeholders like {param}, e.g. "users://{id}/profile"
   * @param handler    Function to execute when the resource is accessed
   * @param definition Resource definition (should include arguments if template)
   * @return ZIO effect that completes with Unit on success or fails with ResourceRegistrationError
   */
  def addResourceTemplate(
    ...
    ZIO.attempt {
      ...
      val templateDefinition = definition.copy(isTemplate = true) // Arguments should be in the passed definition
      ...
      if resourceTemplates.containsKey(uriPattern) then
        ...
        java.lang.System.err.println(s"[ResourceManager] Warning: Resource template with pattern '$uriPattern' already exists. Overwriting.")
        ...
        findMatchingTemplate(uri) match
          case Some((_, _, handler, params)) =>
            handler(params) // Template handler takes extracted params
              .mapError(e => new ResourceAccessError(s"Error accessing templated resource '$uri' with params $params", e))
          // 3. If no match found
          case None =>
   ...
   *
   * @param uri URI to match against templates (e.g. "users://123/profile")
   * @return Option containing the template pattern, definition, handler, and extracted parameters if found
   */
  def findMatchingTemplate(uri: String)
      : Option[(ResourceTemplatePattern, ResourceDefinition, ResourceTemplateHandler, Map[String, String])] =
    resourceTemplates.entrySet().asScala
      ...
      .map { entry =>
        ...
        pattern.matches(uri).map(regexMatch => (pattern, definition, handler, pattern.extractParams(uri, regexMatch)))
      }
...
/**
 * Represents a URI pattern with placeholders. We assume "scheme://..." style,
 * for example: "users://{id}/profile". We anchor the pattern with ^...$ so we must match fully. 
 * */
case class ResourceTemplatePattern(pattern: String):
  ...
  /**
   * Extracts parameters from a given URI based on the template pattern.
   * Assumes the URI has already been matched using `matches`.
   *
   * @param uri The URI to extract parameters from.
   * @param regexMatch The successful Regex.Match object from `matches`.
   ...
   * @return An Option containing the Regex.Match object if it matches, None otherwise.
   */
