...
object McpToolRegistry:
  ...
  extension (server: FastMCPScala)
     ...
     * NOTE: This method is now less relevant as annotation processing handles schema generation.
     * Consider using annotations or registerToolWithSchema directly.
     * 
    ...
    def registerTool[I: JsonEncoder: JsonDecoder: ClassTag, O: JsonEncoder: JsonDecoder](
      ...
      // Generate schema string using a placeholder or basic schema
      // The primary way to get schemas is now via JsonSchemaMacro through annotations
      val schemaString = """{"type":"object","properties":{},"description":"Schema generation via registerTool is basic. Use annotations for detailed schemas."}"""
      ...
      ZIO.attempt {
        ...
        val definition = ToolDefinition(
          ...
          inputSchema = Right(schemaString) // Use Right for string schema
        )

        // Log the schema for debugging
        JSystem.err.println(s"[McpToolRegistry] Registering tool '$toolName' with basic schema (use annotations for full schema):\n$schemaString")
    ...
    /**
     * Register an McpTool with custom schema string.
     * This allows manual specification of the schema string.
     * 
     * @param tool The McpTool implementation to register
     * @param schemaString The JSON schema string for the input type
     * @param name Optional tool name (defaults to tool class name)
     * @param description Optional tool description
     * @param options Tool registration options
     * @return ZIO effect that completes with the server instance
    ...
    def registerToolWithCustomSchema[I: JsonEncoder: JsonDecoder: ClassTag, O: JsonEncoder: JsonDecoder](
      tool: McpTool[I, O],
      schemaString: String, // Changed from schemaBuilder function to direct string
      name: Option[String] = None,
      description: Option[String] = None,
      options: ToolRegistrationOptions = ToolRegistrationOptions()
    ): ZIO[Any, Throwable, FastMCPScala] =
