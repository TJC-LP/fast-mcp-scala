...
object JacksonConverter:
  ...
  private def doConvert[T: ClassTag](name: String, rawValue: Any, tpe: String, mapper: JsonMapper & ClassTagExtensions): T =
    try mapper.convertValue[T](rawValue)
    catch case e: Exception =>
      throw new RuntimeException(
        s"Failed to convert value for parameter '$name' to type $tpe. Value: $rawValue", e
      )
  ...
  given [V: ClassTag](using JacksonConverter[V]): JacksonConverter[Map[String, V]] with
    def convert(name: String, rawValue: Any, mapper: JsonMapper & ClassTagExtensions): Map[String, V] =
