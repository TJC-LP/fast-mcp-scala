...
object JsonSchemaMacro:
  ...
  private def schemaForFunctionArgsImpl[F: Type](fn: Expr[F])(using Quotes): Expr[Json] = {
    ...
    /** Optionally give a name to a Tapir Schema[T]. If T is a product type, Tapir can then track it
    ...
    /** Recursively extracts the parameter list (paramName + paramType). If we detect a function
      * method reference or partial application, we might pick up the real param names.
      */
    @tailrec
    def extractParams(
        tpe: TypeRepr,
        maybeParamNames: Option[List[String]]
    ): List[(String, TypeRepr)] = {
      tpe.widen match {
    ...
    val productFieldsExpr: List[Expr[SProductField[Unit]]] = params.map {
