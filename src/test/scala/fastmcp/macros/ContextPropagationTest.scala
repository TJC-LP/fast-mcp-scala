...
import zio.*
import io.modelcontextprotocol.spec.{McpSchema, McpServerSession}
...
class ContextPropagationTest extends AnyFunSuite:
  ...
  test("Context is automatically injected into function calls") {
    ...
    val server = new FastMCPScala("ContextTestServer")
  ...
  test("@Tool annotations properly handle context parameters") {
    ...
    val server = new FastMCPScala("AnnotationContextTest")
