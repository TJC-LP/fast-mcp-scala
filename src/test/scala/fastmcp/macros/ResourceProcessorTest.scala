...
/**
 * Integration test for Resource annotation and processor
 * This tests the full workflow of resource annotation processing
 */
class ResourceProcessorTest extends AnyFunSuite {
  ...
  test("should correctly access templated resources with parameters") {
    ...
    val userResult = Unsafe.unsafe { implicit unsafe =>
      Runtime.default.unsafe.run {
        for {
          matchResult <- ZIO.fromOption(testServer.resourceManager.findMatchingTemplate("users://123"))
    ...
    val itemResult = Unsafe.unsafe { implicit unsafe =>
      Runtime.default.unsafe.run {
        for {
          matchResult <- ZIO.fromOption(testServer.resourceManager.findMatchingTemplate("items://electronics/phone-123")) 
          (_, definition, handler, params) = matchResult
...
/**
 * Companion object for ResourceProcessorTest containing the resources
 * to be processed by annotations
 */
